...
router.get('/', async (req, res) => {
  try {
    const col = getDb().collection('pagos');
    const filter = {};
    if (req.query.contratoId) filter.contratoId = req.query.contratoId;
...
router.get('/contrato/:contratoId', async (req, res) => {
  try {
    const col = getDb().collection('pagos');
    const list = await col.find({ contratoId: req.params.contratoId }).toArray();
    res.json(list.map(toResponse));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
